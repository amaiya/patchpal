...
def get_openai_auth_path() -> Path:
    return Path.home() / ".patchpal" / "openai_auth.json"


def load_openai_credentials() -> Optional[OpenAICredentials]:
...
async def _start_callback_server(state: str) -> tuple:
...
def _parse_manual_input(input_text: str) -> tuple:
...
async def login(
    ...
    callback_awaitable: Optional[asyncio.Future] = None
    manual_task: Optional[asyncio.Task] = None
...
async def get_valid_openai_token() -> Optional[str]:
    import time
