...
class CopilotCredentials:
    ...
    enterprise_domain: Optional[str] = None  # For GitHub Enterprise
...
def get_copilot_auth_path() -> Path:
    return Path.home() / ".patchpal" / "copilot_auth.json"


def load_copilot_credentials() -> Optional[CopilotCredentials]:
...
def save_copilot_credentials(creds: CopilotCredentials) -> None:
    path = get_copilot_auth_path()
...
def clear_copilot_credentials() -> None:
    path = get_copilot_auth_path()
    if path.exists():
        path.unlink()
...
def is_copilot_logged_in() -> bool:
    return load_copilot_credentials() is not None


def _get_urls(domain: str) -> dict:
    return {
...
def get_base_url_from_token(token: str, enterprise_domain: Optional[str] = None) -> str:
    """
...
async def exchange_for_copilot_token(github_token: str, domain: str = "github.com") -> tuple:
    """
...
async def refresh_copilot_token(creds: CopilotCredentials) -> CopilotCredentials:
    ...
    save_copilot_credentials(new_creds)
    return new_creds


async def get_valid_copilot_token() -> Optional[str]:
    """
